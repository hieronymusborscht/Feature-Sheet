...
public class PostgresConnector {
	...
	public static java.util.TreeMap<String, rg.ent.NewUser> loadAgents(String key_field){
		rg.ent.NewUser usr= null;
		java.util.TreeMap<String, rg.ent.NewUser> users_arr = new java.util.TreeMap<String, rg.ent.NewUser>();
		try{
			connection = getConnection();		
			PreparedStatement prepstmt = connection.prepareStatement("select m_id,img_id,first_name, last_name, email, pass_hash, role, areas_serviced, description, languages, salesforce_id,phone, visible, mem_login, datecreated, web_site, link_text,admin_level, thumb_320 from member where visible='true' order by ?"); 
			...
			while(rs.next()){
				...
				users_arr.put(usr.getStringValue(key_field), usr);	
			}
...
/*	public static java.util.TreeMap<String, rg.ent.ListingAddress> loadAddressList(String s ){
	...
	*/
	/*
	public static java.util.TreeMap<String, NewSummaryListing>  doListingsSearch_b(String sql_statement){
		
		java.util.TreeMap<String, NewSummaryListing> search_results = new java.util.TreeMap<String, NewSummaryListing>();
		try{
		connection = getConnection();
	
		PreparedStatement prepstmt = connection.prepareStatement(sql_statement); 
		ResultSet rs0=  prepstmt.executeQuery();
		NewSummaryListing sl = null;
		while(rs0.next()){	
			
				sl = new NewSummaryListing();
				
				sl.setInt_field("l_listingid",rs0.getInt("l_listingid")); //sl.setL_listingid(rs0.getInt("l_listingid"));
				sl.setString_field("l_displayid", rs0.getString("l_displayid"));
				sl.setAskingprice(rs0.getBigDecimal("l_askingprice")); 		
				sl.setInt_field("sqft",rs0.getInt("sqft")); 
				sl.setInt_field("bedrooms",rs0.getInt("lm_int1_4"));  //bedrooms
				sl.setInt_field("full_baths",rs0.getInt("lm_int1_18"));  //bathrooms	
				sl.setString_field("lmd_mp_latitude",rs0.getString("lmd_mp_latitude"));
				sl.setString_field("lmd_mp_longitude",rs0.getString("lmd_mp_longitude"));
				sl.setString_field("l_address",rg.util.StringCleaner.cleanStreetAddress(rs0.getString("l_address")));
				sl.setString_field("l_city",rs0.getString("l_city"));
				sl.setString_field("l_area",rs0.getString("l_area"));
				sl.setString_field("property_type",rs0.getString("lm_char10_11")); // property type

				sl.setString_field("neighborhood",rs0.getString("lm_char10_5"));
				sl.setString_field("lr_remarks22",rs0.getString("lr_remarks22"));
				sl.setString_field("lr_remarks33",rs0.getString("lr_remarks33"));
				sl.setString_field("l_state",rs0.getString("l_state"));
				sl.setString_field("lo1_organizationname",rs0.getString("lo1_organizationname"));
				sl.setInt_field("number_of_pics", Integer.parseInt(rs0.getString("l_picturecount")));
				sl.setString_field("view",rs0.getString("lm_char100_3"));   //view
	
				sl.setInt_field("days_on",rs0.getInt("days_on"));
				sl.setString_field("six_forty_path",rs0.getString("six_forty_path"));
				sl.setString_field("three_twenty_path",rs0.getString("three_twenty_path"));
				sl.setString_field("one_sixty_path", rs0.getString("one_sixty_path"));
				sl.setString_field("postal", rs0.getString("l_zip"));
